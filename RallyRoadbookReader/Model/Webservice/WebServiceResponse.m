...
@synthesize scanoperationQueue = _scanoperationQueue;
...
- (id)init;
{
    if ((self = [super init]))
    {
        ...
        _scanoperationQueue.maxConcurrentOperationCount = 1;
        
        reachability = [Reachability reachabilityWithHostName:ServerPath];
...
//            NSLog(@"URL Request:%@",URLRequest);
            ...
            NSURLSessionDataTask *task = [session dataTaskWithRequest:URLRequest
                                          ...
                                          ^(NSData *data, NSURLResponse *response, NSError *error) {
                                              ...
                                              dispatch_sync(dispatch_get_main_queue(), ^{
                                                  [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                                                      if (error) {
                                                          resBlock(error, responseObjects, responseString, data);
                                                      }
                                                      else {
                                                          resBlock(error, responseObjects, responseString, data);
                                                      }
    ...
    [operation setQueuePriority:NSOperationQueuePriorityNormal];
    [self.scanoperationQueue addOperation:operation];
}
